...
namespace Reader
{
    ...
    {
        public byte btPacketType;     //数据包头，默认为0xA0
        public byte btDataLen;        //数据包长度，数据包从‘长度’字节后面开始的字节数，不包含‘长度’字节本身
        public byte btReadId;         //读写器地址
        public byte btCmd;            //数据包命令代码
        public byte[] btAryData;      //数据包命令参数，部分命令无参数
        public byte btCheck;          //校验和，除校验和本身外所有字节的校验和
